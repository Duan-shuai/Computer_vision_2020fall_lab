...
class EdgeDetection:
    ...
    def Laplacian(self, filename1, filename2):
        bmp = ReadBmp(filename1)
        # 阈值，根据实验结果大致调整的
        Gmax = 60
        for i in range(bmp.biHeight*bmp.biWidth):
        ...
        for j in range(bmp.biHeight):
            for i in range(bmp.biWidth):
                if i == 0 or j == 0 or i == bmp.biWidth - 1 or j == bmp.biHeight - 1:
                    new_data.append(0)
                    continue
                G = bmp.data[(j + 1) * bmp.biWidth + i - 1][0] + bmp.data[(j + 1) * bmp.biWidth + i + 1][0] \
                    + bmp.data[(j + 1) * bmp.biWidth + i][0] + bmp.data[(j - 1) * bmp.biWidth + i - 1][0] \
                    + bmp.data[(j - 1) * bmp.biWidth + i][0] + bmp.data[(j - 1) * bmp.biWidth + i + 1][0] \
                    + bmp.data[j * bmp.biWidth + i - 1][0] + bmp.data[j * bmp.biWidth + i + 1][0] \
                    + (-8) * bmp.data[j * bmp.biWidth + i][0]
